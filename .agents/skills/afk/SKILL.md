...
Treat `state/.subsuper-escalations`, its `.since` sidecar, `state/.subsuper-last-unconfirmed-inject`, and `state/.subsuper-inject-wedged` as session-scoped delivery artifacts, not as the durable work record.
Clear the duplicate-digest marker with the buffer whose delivery attempt it describes; `bin/fm-afk-start.sh`'s `fm_afk_clear_stale_artifacts` owns why.
