...
# composer could otherwise keep the marker alive indefinitely. A consecutive
# unreadable-observation count bounds that. The time window cannot do this job:
# in the case the guard exists for, the earlier Enter landed and the agent is
# mid-turn, so polls return at the busy guard above both the revocation and the
# dedup, and the duplicate only arrives at the next composer-empty poll once
# that turn ends - measured at roughly one every 976s in the incident that
# motivated the guard. A busy pane is deliberately NOT one of these
# observations, so a long legitimate turn never spends the budget.
test_inject_msg_dedup_marker_expires_after_consecutive_unreadable_reads() {
  local dir state marker
  dir=$(make_supercase inject-dedup-unreadable)
  state="$dir/state"
  marker="$state/.subsuper-last-unconfirmed-inject"
  afk_enter "$state"
  (
    ...
    # Below the bound the marker survives and still suppresses the duplicate,
    # so the feature the guard exists for keeps working.
    arm_marker "digest one"
    unreadable_poll "digest one"
    [ -f "$marker" ] || fail "one unreadable observation must not expire the duplicate-digest marker"
    unreadable_poll "digest one"
    [ -f "$marker" ] || fail "two unreadable observations must not expire the duplicate-digest marker"
    fm_backend_composer_state() { printf 'empty'; }
    fm_backend_send_text_submit() { fail "an identical digest inside the bound must not be retyped"; }
    FM_SUPERVISOR_BACKEND=herdr FM_SUPERVISOR_TARGET="default:w1:p2" inject_msg "digest one" "$state" \
      || fail "an identical digest still inside the unreadable bound must dedup"
    # Reaching the bound expires it: the escalation is retyped, never truncated.
    arm_marker "digest two"
    unreadable_poll "digest two"
    unreadable_poll "digest two"
    unreadable_poll "digest two"
    [ ! -f "$marker" ] \
      || fail "three consecutive unreadable observations must expire the duplicate-digest marker"
    fm_backend_composer_state() { printf 'empty'; }
    fm_backend_send_text_submit() { printf 'typed\n' >> "$dir/typed"; printf 'empty'; }
    FM_SUPERVISOR_BACKEND=herdr FM_SUPERVISOR_TARGET="default:w1:p2" inject_msg "digest two" "$state" \
      || fail "the retyped digest must report delivered once the submit confirms"
    [ -s "$dir/typed" ] \
      || fail "a marker that survived an unreadable stretch still vouched for a delivery nothing ever confirmed"
    # A busy pane returns above the composer read, so it must not spend budget.
    arm_marker "digest three"
    pane_is_busy() { return 0; }
    fm_backend_composer_state() { fail "composer_state must not be consulted once the busy guard deferred"; }
    local i=0
    while [ "$i" -lt 5 ]; do
      if FM_SUPERVISOR_BACKEND=herdr FM_SUPERVISOR_TARGET="default:w1:p2" inject_msg "digest three" "$state"; then
        fail "a busy supervisor pane must defer"
      fi
      i=$((i + 1))
    done
    [ -f "$marker" ] \
      || fail "a busy supervisor pane is not an unreadable observation and must not expire the marker"
    pane_is_busy() { return 1; }
    fm_backend_composer_state() { printf 'empty'; }
    fm_backend_send_text_submit() { fail "the duplicate must still be suppressed after a long busy turn"; }
    FM_SUPERVISOR_BACKEND=herdr FM_SUPERVISOR_TARGET="default:w1:p2" inject_msg "digest three" "$state" \
      || fail "the duplicate arriving after a long busy turn must still dedup"
  ) || fail "unreadable-bound inject_msg subshell failed"
  pass "inject_msg: the duplicate-digest marker expires after consecutive unreadable composer reads, while a busy turn never spends that budget"
}
