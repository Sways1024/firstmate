...
test_return_gate_orders_catchup_before_bearings() {
  ...
  printf '0123456789abcdef0123456789abcdef %s\n' "$(date +%s)" > "$dir/home/state/.subsuper-last-unconfirmed-inject"
  ...
  [ ! -e "$dir/home/state/.subsuper-last-unconfirmed-inject" ] || fail "successful check left the duplicate-digest marker behind to suppress a later identical digest"
