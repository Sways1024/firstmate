...
test_respawn_defaults_from_recorded_meta() {
  ...
  id=respawn-meta-z1
  rec=$(make_spawn_case respawn-meta claude "$id")
  read_case_record "$rec"
  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --model claude-opus-5 --effort high)
  status=$?
  expect_code 0 "$status" "initial spawn with an explicit profile should succeed"
  meta="$HOME_DIR/state/$id.meta"
  assert_meta_profile "$meta" claude claude-opus-5 high
  # Config changes after the launch. A bare respawn of the SAME task (recovery
  # after a backend restart) must return the worker with the task's own
  # recorded launch profile, not re-resolve from today's config (#1571 gap B).
